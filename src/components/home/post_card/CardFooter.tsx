...
const CardFooter = ({ post }: any) => {
    ...
    const { auth, theme, socket }: any = useSelector(state => state)
    ...
    // Likes
    useEffect(() => {
        if (post.likes.find((like: { _id: string }) => like._id === auth.user._id)) {
            setIsLike(true)
        } else {
            setIsLike(false)
        }
    ...
    // Saved
    useEffect(() => {
        if (auth.user.saved.find((id: string) => id === post._id)) {
            setSaved(true)
        } else {
            setSaved(false)
        }
